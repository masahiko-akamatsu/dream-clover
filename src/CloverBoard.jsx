  ...
  return (
    <div style={s.wrap}>
      <style>{`
        ...
        textarea::placeholder{color:#aaa;font-size:12px}
        textarea:focus{outline:none;border-color:#5DCAA5 !important;box-shadow:0 0 0 2px rgba(93,202,165,0.2)}
...
function SectionCard({ sec, value, onChange }) {
  return (
    <div style={{...s.card, borderTop:`4px solid ${sec.color}`, background:sec.bg}}>
      ...
      <textarea style={s.textarea} value={value} onChange={e=>onChange(e.target.value)} placeholder={sec.ph} />
    </div>
  );
}
...
const s = {
  ...
  header:{background:'#fff',borderBottom:'2px solid #c8e8dc',padding:'0.7rem 1.2rem',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:'0.6rem',position:'sticky',top:0,zIndex:100,boxShadow:'0 2px 8px rgba(29,158,117,0.08)'},
  ...
  textarea:{width:'100%',minHeight:155,border:'1.5px solid rgba(0,0,0,0.1)',borderRadius:10,padding:'9px 11px',fontFamily:"'Noto Sans JP',sans-serif",fontSize:13,lineHeight:1.8,resize:'vertical',background:'rgba(255,255,255,0.85)',color:'#1a1a1a'},
  ...
  centerTitle:{fontSize:13,fontWeight:700,color:'#0F6E56',letterSpacing:1,marginBottom:'0.8rem',textAlign:'center'},
